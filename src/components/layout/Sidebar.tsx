...
export function Sidebar({ contentPath, onSelectFile, selectedFilePath, onNewFile, drafts }: SidebarProps) {
  ...
  const [activeSidebarTab, setActiveSidebarTab] = useState<'content' | 'git'>('content');
      ...
      <div className="px-4 pt-4 shrink-0">
        <div className="flex bg-[#121212] rounded-lg p-1 border border-white/5 shadow-inner">
           <button 
             ...
             className={`flex-1 py-1.5 text-[11px] font-bold rounded-md transition-all uppercase tracking-wider ${activeSidebarTab === 'content' ? 'bg-white/10 text-white shadow-sm' : 'text-white/40 hover:text-white/80'}`}
           >
             Content
           </button>
           <button 
             onClick={() => setActiveSidebarTab('git')}
             className={`flex-1 py-1.5 text-[11px] font-bold rounded-md transition-all uppercase tracking-wider flex items-center justify-center gap-1 ${activeSidebarTab === 'git' ? 'bg-white/10 text-white shadow-sm' : 'text-white/40 hover:text-white/80'}`}
           >
             Git
           </button>
        </div>
      ...
      <div className="flex-1 flex flex-col pt-4 overflow-hidden relative">
        ...
        {activeSidebarTab === 'content' ? (
          <>
            ...
            <div className="mt-auto pt-4 border-t border-border-low shrink-0 flex flex-col gap-4 px-4 pb-4">
              <div className="px-2">
                <Button 
                  onClick={onNewFile}
          ...
          <GitPanel contentPath={contentPath} onSelectFile={onSelectFile} />
        )}
      </div>
    </aside>
  );
